...
interface NewGameDialogProps {
  open: boolean;
  onClose: () => void;
  onCreateGame: (players: [PlayerSlot, PlayerSlot, PlayerSlot, PlayerSlot]) => void;
}
...
const NewGameDialog = ({ open, onClose, onCreateGame }: NewGameDialogProps) => {
  ...
  const buildSlot = (selected: PlayerSelection, inputText: string, fallbackN: number): PlayerSlot => {
    if (selected) {
      return { uid: selected.uid, guestName: null };
    }
    const name = inputText.trim();
    return { uid: null, guestName: name || `${t('newGame.player', { n: fallbackN })}` };
  };
  ...
  const handleStart = () => {
    ...
    // Reset
    setPlayer2(null); setPlayer2Input('');
    setPlayer3(null); setPlayer3Input('');
    setPlayer4(null); setPlayer4Input('');
  };
    ...
    return (
      ...
      onChange={(_, newVal) => {
        ...
        if (typeof newVal === 'string' || newVal === null) {
          ...
          if (n < 4) {
            setTimeout(() => {
              const nextInput = document.getElementById(`player${n + 1}-input`);
              if (nextInput) {
      ...
      renderOption={(props, option) => {
        ...
        return (
          <Box component="li" key={key} {...rest}
            ...
            <Typography sx={{ fontSize: '1.2rem' }}>{option.avatar}</Typography>
      ...
      <DialogContent>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Chip
              icon={<PersonIcon />}
              label={`${playerLabel(1)}: ${profile?.displayName || 'You'}`}
              color="primary"
              variant="outlined"
          ...
          {/* Player 2 */}
          {renderPlayerPicker(2, player2, player2Input, setPlayer2, setPlayer2Input)}

          {/* Team 2 header */}
          <Typography variant="subtitle2" color="secondary" sx={{ fontWeight: 600, mt: 1 }}>
            {t('game.team2')}
          </Typography>

          {/* Player 3 */}
          {renderPlayerPicker(3, player3, player3Input, setPlayer3, setPlayer3Input)}

          {/* Player 4 */}
          {renderPlayerPicker(4, player4, player4Input, setPlayer4, setPlayer4Input)}
        </Box>
      </DialogContent>
